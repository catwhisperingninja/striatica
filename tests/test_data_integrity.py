...
class TestCircuitIntegrity:
    """Ensure circuit data is structurally valid and aligns with geometric data."""

    def test_circuit_node_indices_in_range(self):
        """All circuit node featureIndex values must be < numFeatures.

        The circuit JSONs under frontend/public/data/circuits/ are the
        gpt2-small demo circuits: generated by process_gpt2_small.py from
        gpt2-small-6-res-jb.json (see CLAUDE.md, "Circuit Data — Two Separate
        File Paths"). Their node indices run up to ~24.3k (e.g. sim-23123 seeds
        on gpt2 feature #23123), which fits only the gpt2-small feature count
        (24,576) — NOT the gemma datasets (16,384). So we validate against that
        specific dataset by identity, not against an arbitrary *.json or the
        first "big enough" one (pythia at 32,768 would also fit index 24,311 —
        that's the trap). Sidecar files (*-metadata.json, *-validation.json,
        datasets.json) and the circuit/manifest JSONs are not dataset files and
        ...
        # Exclude non-dataset JSONs: sidecars, the top-level manifest, and any
        # circuit files. (glob("*.json") is non-recursive, so circuit files in
        # the circuits/ subdir aren't matched — the "circuit" guard is belt-and-
        # suspenders in case a circuit JSON is ever written to the data root.)
        def _is_dataset(path: Path) -> bool:
            name = path.name
            if name in ("manifest.json", "datasets.json"):
                return False
            if name.endswith("-metadata.json") or name.endswith("-validation.json"):
                return False
            if "circuit" in name:
                return False
            return True

        dataset_files = [f for f in data_dir.glob("*.json") if _is_dataset(f)]

        # These circuits belong to the gpt2-small demo dataset specifically.
        gpt2_datasets = [f for f in dataset_files if f.name.startswith("gpt2-small")]
        if not gpt2_datasets:
            pytest.skip("gpt2-small dataset (source of these circuits) not present")
        dataset_path = gpt2_datasets[0]
        ...
        for circuit_path in circuits_dir.glob("*.json"):
            ...
            for node in circuit["nodes"]:
                assert 0 <= node["featureIndex"] < num_features, (
                    ...
                    f"{node['featureIndex']} out of range [0, {num_features}) "
                    f"(validated against {dataset_path.name})"
                )
